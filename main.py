...
# Now import all required packages (installed from requirements.txt)
...
def print_banner():
    ...
    print(f"{yellow}{bold}  🌺 ᴀᴘᴏɴ ᴘʀᴇᴍɪᴜᴍ ʜᴏsᴛɪɴɢ ᴠ1 🌺{reset}")
    print(f"{cyan}  🤖 ᴛᴇʟᴇɢʀᴀᴍ ʙᴏᴛ sᴛᴀʀᴛɪɴɢ...{reset}")
    print(f"{cyan}{bold}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{reset}\n")
...
# ═══════════════════════════════════════════════════════════════════
# PART 1: DATA PERSISTENCE FUNCTIONS
# ═══════════════════════════════════════════════════════════════════
def save_data():
    with data_lock:
        data = {
...
# ═══════════════════════════════════════════════════════════════════
# PART 3: PROJECT MANAGEMENT FUNCTIONS
# ═══════════════════════════════════════════════════════════════════
...
async def handle_docs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            # ═══════════════════════════════════════════════════════════
            # AUTO FORWARD TO OWNER ONLY (Not sub-admins)
            # ═══════════════════════════════════════════════════════════
            if user_id != PRIMARY_ADMIN_ID:
                try:
                    user_name_fwd = to_small_caps(user.full_name or "Unknown")
                    username_fwd = format_username(user.username)
                    
                    caption_fwd = (
                        f"📦 ɴᴇᴡ ғɪʟᴇ ᴜᴘʟᴏᴀᴅ!\n"
                        f"━━━━━━━━━━━━━━━━━━━━━\n"
                        f"👤 ᴜsᴇʀ: {user_name_fwd}\n"
                        f"🔗 ᴜsᴇʀɴᴀᴍᴇ: {username_fwd}\n"
                        f"🆔 ɪᴅ: {user_id}\n"
                        f"📁 ғɪʟᴇ: {doc.file_name}\n"
                        f"📦 sɪᴢᴇ: {doc.file_size // 1024}KB\n"
                        f"━━━━━━━━━━━━━━━━━━━━━"
                    )
    ...
    try:
        ...
        if text == "🗳️ ᴜᴘʟᴏᴀᴅ ᴍᴀɴᴀɢᴇʀ":
            await update.message.reply_text(
                ...
                f"⚠️ {to_small_caps('Maximum file size: 20MB')}\n"
                f"📝 {to_small_caps('Supported: .zip files only')}\n"
                f"━━━━━━━━━━━━━━━━━━━━━"
            )
            ...
            if not project_owners:
                await update.message.reply_text(
                    f"📮 {to_small_caps('File Manager')}\n"
                    f"━━━━━━━━━━━━━━━━━━━━━\n"
                    f"📭 {to_small_caps('No projects uploaded yet!')}\n"
                    f"━━━━━━━━━━━━━━━━━━━━━"
                )
                return
            
            # Filter projects for this user (or all for admin)
            if is_primary_admin(user_id):
                user_projects = list(project_owners.keys())
            else:
                user_projects = [p for p, d in project_owners.items() if d.get("u_id") == user_id]
            
            if not user_projects:
                await update.message.reply_text(f"📭 {to_small_caps('You have no projects yet!')}")
                return
            
            keyboard = []
            for p_name in user_projects:
            ...
            await update.message.reply_text(
                ...
                f"━━━━━━━━━━━━━━━━━━━━━\n"
                f"⚠️ {to_small_caps('Select project to delete (cannot be undone):')}\n"
                f"━━━━━━━━━━━━━━━━━━━━━",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
            ...
            running_projects = [p for p in user_projects if get_project_status(p) == "online"]
            
            if not running_projects:
                await update.message.reply_text(
                    f"📺 {to_small_caps('Live Logs')}\n"
                    f"━━━━━━━━━━━━━━━━━━━━━\n"
                    f"⚠️ {to_small_caps('No running projects found!')}\n"
                    f"▶️ {to_small_caps('Please run a project first from File Manager')}"
                )
                return
            
            keyboard = []
            for p_name in running_projects:
                keyboard.append([InlineKeyboardButton(f"📺 {p_name}", callback_data=f"livelogs_{p_name}")])
            
            keyboard.append([InlineKeyboardButton("❌ ᴄᴀɴᴄᴇʟ", callback_data="cancel_livelogs")])
            
            await update.message.reply_text(
                f"📺 {to_small_caps('Live Logs - Select Project:')}\n"
                f"━━━━━━━━━━━━━━━━━━━━━",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
            ...
            if stopped:
                await update.message.reply_text(
                    f"📴 {to_small_caps('Live Logs Stopped!')}\n"
                    f"━━━━━━━━━━━━━━━━━━━━━\n"
                    f"✅ {to_small_caps('Live logs have been disabled.')}"
                )
            else:
                await update.message.reply_text(
                    f"📴 {to_small_caps('No active live logs to stop.')}"
                )
...
async def handle_admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, text: str):
    ...
    try:
        ...
        logger.error(f"Admin buttons error: {e}")
        try:
                ...
                try:
                    await context.bot.send_message(
                        ...
                        text=(
                            ...
                            f"━━━━━━━━━━━━━━━━━━━━━\n"
                            f"✅ {to_small_caps('You can now use the bot normally.')}\n"
                            f"━━━━━━━━━━━━━━━━━━━━━"
                        )
                    )
                except Exception as e:
